...
export default function ThemeSection() {
  ...
  return (
    <div style={{
      ...
      <div>
        <p style={{ fontSize: '0.9375rem', fontWeight: 500, color: 'var(--color-label-primary)', margin: 0 }}>Theme</p>
        <p style={{ fontSize: '0.8125rem', color: 'var(--color-label-tertiary)', marginTop: '2px' }}>
          {theme === 'light' ? 'Light mode active' : 'Dark mode active'}
        </p>
      </div>
      <div style={{ display: 'flex', gap: '6px', background: 'var(--color-bg)', borderRadius: '10px', padding: '4px' }}>
        {['light', 'dark'].map(t => (
          <button
            key={t}
            onClick={() => setTheme(t)}
