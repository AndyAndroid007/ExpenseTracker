...
export default function MessageThread({ messages, onConfirm, onEdit }) {
  ...
  return (
    <div ref={bottomRef} style={{ flex: 1, overflowY: 'auto', padding: '1rem 0', display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
      {messages.map((msg, i) => {
        if (msg.type === 'streak') {
        ...
        if (msg.type === 'confirm_card') {
          return (
            <div key={i} style={{ maxWidth: '80%', alignSelf: 'flex-start' }}>
        ...
        return (
          <div
            key={i}
            // className={`w-fit max-w-[80%] px-3 py-2 text-[15px] leading-[1.4] ${isUser ? 'self-end' : 'self-start'} ${isUser ? 'bg-[var(--color-primary)] text-white rounded-[18px] rounded-tr-[4px]' : 'bg-[var(--color-surface)] text-[var(--color-label-primary)] border border-[#e5e5ea] rounded-2xl rounded-tl-[4px]'}`}
            style={{
              maxWidth: '80%',
